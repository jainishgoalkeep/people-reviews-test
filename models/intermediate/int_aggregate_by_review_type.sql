...
with
    ...
    aggregate_by_review_type as (
        select
            ...
            case
                when review_type = '1 - For Self'
                then '1 - Overall Rating From Self'
                when review_type = '2 - For Peer'
                then '2 - Overall Rating From Peer'
                when review_type = '4 - For Manager'
                then '3 - Overall Rating From Managee'
                when review_type = '3 - For Managee'
                then '4 - Overall Rating From Manager'
                else null
            end as long_form,
